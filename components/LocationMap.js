...
export default function LocationMap({ locations=[] }){
    ...
    locations.forEach(loc => {
      if (loc.connected) {
        loc.connected.forEach(connectedId => {
          if (edges.find(e => (e.from===loc.id && e.to===connectedId)||(e.from===connectedId && e.to===loc.id))) return
  ...
  return <div ref={containerRef} style={{height:400,background:'#0b1220',borderRadius:8}} />
}
