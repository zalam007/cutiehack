...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    const items = await prisma.storyEvent.findMany({
  ...
  if (req.method === "POST") {
    ...
    const underLimit = await checkEntityLimit('storyEvent', data.worldId);
    if (!underLimit) {
      res.status(403).json({ error: "Maximum 10 events per world. Delete one to create a new one." });
      return;
    }
