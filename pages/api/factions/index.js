...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    const underLimit = await checkEntityLimit('faction', data.worldId);
    if (!underLimit) {
      res.status(403).json({ error: "Maximum 10 factions per world. Delete one to create a new one." });
      return;
    }

    const created = await prisma.faction.create({ data });
